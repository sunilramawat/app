    ...
    <div class="col-lg-12">
      ...
      <div class="panel panel-default">
        <div class="panel-heading">
          Users List<?php //echo $this->Html->link(__('Add Page'), ['action' => 'add'],['class'=>'btn btn-primary btn-sm float_Right']); ?>
        </div>
        <!-- /.panel-heading -->
...
<?php $url = (['controller' => 'users', 'action' => 'index', 'prefix'=>'admin']); ?>
   ...
   table = $('#list').DataTable({
      ...
      "columns": [
         // { "name": "p_id","orderable":false,"searchable":false,'width':'5%', 'sClass': 'text-center'},
          { "name": "username", 'width':'15%',"searchable":true,'regex':true ,'sClass': 'text-left'},
          { "name": "email","searchable":true, 'width':'15%'},
          
          { "name": "added_date", 'width':'15%'},
...
$('#users_li').addClass('active');
  function changeStatus(id,status){
      URL = '<?php echo $this->Url->Build(["controller" => "users","action" => "change_status","prefix"=>"admin"]);?>';
      $.ajax({
          url : URL,
          type: "POST",
  ...
  function deleteRow(id){
    bootbox.confirm("Are you sure you want to delete user", function(r) {
        if (r == true ) {
            URL = '<?php echo $this->Url->Build(["controller" => "users","action" => "delete_row"]);?>';
            $.ajax({
                url: URL,
                type: 'POST',
                data: ({id: id}),
