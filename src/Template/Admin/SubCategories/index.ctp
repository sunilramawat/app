    ...
    <div class="col-lg-12">
      ...
      <div class="panel panel-default">
        <div class="panel-heading">
          SubCategories List<?php echo $this->Html->link(__('Add SubCategory'), ['action' => 'add'],['class'=>'btn btn-primary btn-sm float_Right']); ?>
        </div>
        <!-- /.panel-heading -->
...
<?php $url = (['controller' => 'sub_categories', 'action' => 'index',$this->request->params['pass'][0], 'prefix'=>'admin']); ?>
...
$(document).ready(function (){
   ...
   table = $('#list').DataTable({
      ...
      "columns": [
          { "name": "sc_id","orderable":false,"searchable":false,'width':'5%', 'sClass': 'text-center'},
          { "name": "sc_name", 'width':'15%',"searchable":true,'regex':true},
...
$('#categories').addClass('active');
  function changeStatus(id,status){
      URL = '<?php echo $this->Url->Build(["controller" => "sub_categories","action" => "change_status","prefix"=>"admin"]);?>';
      $.ajax({
          url : URL,
          type: "POST",
  ...
  function deleteRow(id){
    bootbox.confirm("Are you sure you want to delete user", function(r) {
        if (r == true ) {
            URL = '<?php echo $this->Url->Build(["controller" => "sub_categories","action" => "delete_row"]);?>';
            $.ajax({
                url: URL,
                type: 'POST',
                data: ({id: id}),
